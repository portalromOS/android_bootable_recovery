...
/* recovery dark:  #335600
   recovery light: #618729
   fastbootd dark: #335600
   fastboot light: #618729 */
void ScreenRecoveryUI::SetColor(UIElement e) const {
  switch (e) {
    case UIElement::INFO:
      if (fastbootd_logo_enabled_)
        gr_color(51, 86, 0, 255);
      else
        gr_color(51, 86, 0, 255);
      break;
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : portalrom_logo_;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false) ||
    ...
    portalrom_logo_ = LoadBitmap("logo_image_switch");
    fastbootd_logo_ = LoadBitmap("fastbootd");
  } else {
    portalrom_logo_ = LoadBitmap("logo_image");
  }
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(portalrom_logo_.get());
      const static int logo_height = gr_get_height(portalrom_logo_.get());
